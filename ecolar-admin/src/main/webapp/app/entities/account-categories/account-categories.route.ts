...
export class AccountCategoriesResolve implements Resolve<IAccountCategories> {
    ...
    resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<AccountCategories> {
        const id = route.params['id'] ? route.params['id'] : null;
        if (id) {
            return this.service.find(id).pipe(
                filter((response: HttpResponse<AccountCategories>) => response.ok),
                map((accountCategories: HttpResponse<AccountCategories>) => accountCategories.body)
            );
        }
...
export const accountCategoriesRoute: Routes = [
    {
        ...
        data: {
            ...
            pageTitle: 'ecolarApp.accountCategories.home.title'
        },
