...
export class EAccountResolve implements Resolve<IEAccount> {
    ...
    resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<EAccount> {
        const id = route.params['id'] ? route.params['id'] : null;
        if (id) {
            return this.service.find(id).pipe(
                filter((response: HttpResponse<EAccount>) => response.ok),
                map((eAccount: HttpResponse<EAccount>) => eAccount.body)
            );
        }
