...
export default (state: UserPreferencesState = initialState, action): UserPreferencesState => {
  switch (action.type) {
...
export const getEntities: ICrudGetAllAction<IUserPreferences> = (page, size, sort) => ({
  type: ACTION_TYPES.FETCH_USERPREFERENCES_LIST,
  payload: axios.get<IUserPreferences>(`${apiUrl}?cacheBuster=${new Date().getTime()}`)
});

export const getEntity: ICrudGetAction<IUserPreferences> = id => {
  const requestUrl = `${apiUrl}/${id}`;
  return {
    type: ACTION_TYPES.FETCH_USERPREFERENCES,
    payload: axios.get<IUserPreferences>(requestUrl)
  };
};

export const createEntity: ICrudPutAction<IUserPreferences> = entity => async dispatch => {
...
export const updateEntity: ICrudPutAction<IUserPreferences> = entity => async dispatch => {
  const result = await dispatch({
...
export const deleteEntity: ICrudDeleteAction<IUserPreferences> = id => async dispatch => {
  const requestUrl = `${apiUrl}/${id}`;
  const result = await dispatch({
