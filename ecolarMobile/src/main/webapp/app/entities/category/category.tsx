    ...
    return (
      <div>
        <h2 id="category-heading">
          <Translate contentKey="ecolarMobileApp.category.home.title">Categories</Translate>
          <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
            <FontAwesomeIcon icon="plus" />&nbsp;
            <Translate contentKey="ecolarMobileApp.category.home.createLabel">Create new Category</Translate>
          </Link>
        </h2>
            ...
            <tbody>
              {categoryList.map((category, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>
                    <Translate contentKey={`ecolarMobileApp.AccountType.${category.accountType}`} />
                  </td>
                  <td>{category.parent ? <Link to={`category/${category.parent.id}`}>{category.parent.name}</Link> : ''}</td>
                  <td>{category.document ? <Link to={`account-categories/${category.document.id}`}>{category.document.id}</Link> : ''}</td>
